...
use aptos_types::on_chain_config::{ValidatorSet, OnChainConsensusConfig, ConsensusConfigV1, ProposerElectionType};
use aptos_types::PeerId;
use aptos_types::transaction::Script;
use diesel::IntoSql;
use forge::{reconfig, NodeExt, Swarm, SwarmExt, LocalSwarm};
...
async fn check_vote_to_elected(swarm: &mut LocalSwarm) -> (Option<u64>, Option<u64>) {
    ...
    let (address_off, rest_client_off) = swarm.validators().skip(1).next().map(|v| (v.peer_id(), v.rest_client())).unwrap();
    ...
    let mut epoch = 0;
    for _ in 0..7 {
        epoch = reconfig(&rest_client, &transaction_factory, swarm.chain_info().root_account()).await.epoch;
    }
    ...
    tokio::time::sleep(Duration::from_secs(20)).await;
    let events = FetchMetadata::fetch_new_block_events(&rest_client, Some(epoch as i64), None).await.unwrap();
    assert_eq!(events.len(), 1);

    let info = events.first().unwrap();
    let off_index = info.validators.iter().filter(|v| v.address == address_off).next().unwrap().validator_index;
...
async fn test_onchain_config_change() {
    ...
    let root_cli_index = cli.add_account_with_address_to_cli(swarm.root_key(), swarm.chain_info().root_account().address());

    let rest_client = swarm.validators().next().unwrap().rest_client();
    
    let current_consensus_config: OnChainConsensusConfig = bcs::from_bytes(&rest_client.get_account_resource_bcs::<Vec<u8>>(CORE_CODE_ADDRESS, "0x1::consensus_config::ConsensusConfig").await.unwrap().into_inner()).unwrap();

    let OnChainConsensusConfig::V1(inner) = current_consensus_config;
    let leader_reputation_type = if let ProposerElectionType::LeaderReputationV2(leader_reputation_type) = inner.proposer_election_type {
        leader_reputation_type
    } else {
        panic!()
    };
    let new_consensus_config = OnChainConsensusConfig::V1(ConsensusConfigV1 {
        proposer_election_type: ProposerElectionType::LeaderReputation(leader_reputation_type),
        ..inner
    });
    ...
    println!("Epoch before : {}", rest_client.get_ledger_information().await.unwrap().into_inner().epoch);
    cli.run_script(root_cli_index, &update_consensus_config_script).await.unwrap();
    ...
    println!("Epoch after : {}", rest_client.get_ledger_information().await.unwrap().into_inner().epoch);
    ...
    println!("With old config: {:?} to {:?}", first_vote_old, first_elected_old);

    cli.analyze_validator_performance(Some(0), None).await.unwrap();
}
