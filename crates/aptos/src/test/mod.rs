...
use crate::move_tool::{
    ...
    InitPackage, MemberId, PublishPackage, RunFunction, TestPackage, RunScript,
};
use crate::node::{
...
impl CliTestFramework {
    ...
    pub fn add_account_with_address_to_cli(&mut self, private_key: Ed25519PrivateKey, address: AccountAddress) -> usize {
        self.account_addresses.push(address);
        self.account_keys.push(private_key);
        self.account_keys.len() - 1
    }
    ...
    pub async fn run_script(
        ...
        RunScript {
            txn_options: self.transaction_options(index, None),
            compile_proposal_args: CompileScriptFunction { script_path: Some(source_path), compiled_script_path: None, framework_package_args: FrameworkPackageArgs { framework_git_rev: None, framework_local_dir: Some(Self::aptos_framework_dir()) } },
            args: Vec::new(),
            type_args: Vec::new(),
        }
        .execute()
